...
import { AppContext, Order, User, UserRole, UserPermission } from '../context/AppContext';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ setActiveView }) => {
    const { config, setConfig, language, contactMessages, deleteContactMessage, markContactMessagesAsRead, orders, currentUser, users, updateUserRoleAndPermissions } = useContext(AppContext);
    ...
    const handleRoleChange = (userId: number, newRole: UserRole) => {
        ...
        if (user) {
            updateUserRoleAndPermissions(userId, newRole, user.permissions);
        }
    };
    
    const handlePermissionChange = (userId: number, permission: UserPermission, value: boolean) => {
        const user = users.find(u => u.id === userId);
        if (user && user.role === 'Staff') {
            const newPermissions = { ...user.permissions, [permission]: value };
            updateUserRoleAndPermissions(userId, user.role, newPermissions);
        }
    };
    ...
    const allTabs = [
        ...
        { id: 'users', label: admin.tabUsers[language], icon: <UserIcon className="w-6 h-6" /> },
    ];
    
    const availableTabs = allTabs.filter(tab => {
        if (currentUser?.role === 'Admin') return true;
        if (currentUser?.role === 'Staff') return !!currentUser.permissions[tab.id as UserPermission];
        return false;
    });
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'users': return (
                <div className="p-6 rounded-lg" style={{backgroundColor: 'var(--card-background-color)', borderColor: 'var(--border-color)'}}>
                    <h3 className="text-2xl font-bold mb-6" style={{ color: 'var(--text-strong-color)'}}>{admin.usersTitle[language]}</h3>
                    <div className="space-y-4">
                        {users.map((user: User) => (
                            <div key={user.id} className="p-4 border rounded-lg" style={{borderColor: 'var(--border-color)', backgroundColor: 'var(--background-color)'}}>
                                <div className="grid md:grid-cols-3 gap-4 items-center">
                                    <div>
                                        <div className="font-bold" style={{color: 'var(--text-strong-color)'}}>{user.name}</div>
                                        <div className="text-sm" style={{color: 'var(--text-color)'}}>{user.email}</div>
                                    </div>
                                    <div>
                                        <label htmlFor={`role-${user.id}`} className="sr-only">{admin.role[language]}</label>
                                        <select id={`role-${user.id}`} value={user.role} onChange={(e) => handleRoleChange(user.id, e.target.value as UserRole)} style={inputStyles} className="w-full p-2 border rounded-md">
                                            <option value="Admin">{admin.roleAdmin[language]}</option>
                                            <option value="Staff">{admin.roleStaff[language]}</option>
                                            <option value="Member">{admin.roleMember[language]}</option>
                                        </select>
                                    </div>
                                    <div>
                                        {user.role === 'Staff' && (
                                            <div className="space-y-1">
                                                <h4 className="font-semibold text-sm mb-2">{admin.permissions[language]}:</h4>
                                                {allTabs.filter(t => t.id !== 'users').map(tab => (
                                                    <label key={tab.id} className="flex items-center gap-2 text-sm">
                                                        <input type="checkbox" checked={!!user.permissions[tab.id as UserPermission]} onChange={(e) => handlePermissionChange(user.id, tab.id as UserPermission, e.target.checked)} />
                                                        {tab.label}
                                                    </label>
                                                ))}
                                            </div>
